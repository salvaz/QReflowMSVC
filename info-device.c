...
typedef struct fz_info_device_s fz_info_device;

struct fz_info_device_s
{
	fz_device super;
	fz_info_page *page;
	fz_point pen, start;
...
const char *fz_info_options_usage =
	"Text output options:\n"
...
*/
fz_info_page *
fz_new_info_page(fz_context *ctx, fz_rect mediabox)
{
	fz_pool *pool = fz_new_pool(ctx);
	fz_info_page *page = NULL;
	fz_try(ctx)
	{
...
void
fz_drop_info_page(fz_context *ctx, fz_info_page *page)
{
	if (page)
	{
		fz_info_block *block;
		for (block = page->first_block; block; block = block->next)
            if (block->type == FZ_INFO_BLOCK_IMAGE)
				fz_drop_image(ctx, block->u.i.image);
		fz_drop_pool(ctx, page->pool);
	}
}

static fz_info_block *
add_block_to_page(fz_context *ctx, fz_info_page *page)
{
	fz_info_block *block = fz_pool_alloc(ctx, page->pool, sizeof *page->first_block);
...
static fz_info_block *
add_text_block_to_page(fz_context *ctx, fz_info_page *page)
{
	fz_info_block *block = add_block_to_page(ctx, page);
    block->type = FZ_INFO_BLOCK_TEXT;
	return block;
}

static fz_info_block *
add_image_block_to_page(fz_context *ctx, fz_info_page *page, fz_matrix ctm, fz_image *image)
{
	fz_info_block *block = add_block_to_page(ctx, page);
    block->type = FZ_INFO_BLOCK_IMAGE;
...
static fz_info_line *
add_line_to_block(fz_context *ctx, fz_info_page *page, fz_info_block *block, const fz_point *dir, int wmode)
{
	fz_info_line *line = fz_pool_alloc(ctx, page->pool, sizeof *block->u.t.first_line);
...
static fz_info_char *
add_char_to_line(fz_context *ctx, fz_info_page *page, fz_info_line *line, fz_matrix trm, fz_font *font, float size, int c, fz_point *p, fz_point *q)
{
	fz_info_char *ch = fz_pool_alloc(ctx, page->pool, sizeof *line->first_char);
	fz_point a, d;
...
static void
fz_add_info_char_imp(fz_context *ctx, fz_info_device *dev, fz_font *font, int c, int glyph, fz_matrix trm, float adv, int wmode)
{
	fz_info_page *page = dev->page;
	fz_info_block *cur_block;
	fz_info_line *cur_line;
    ...
    if (cur_block && cur_block->type != FZ_INFO_BLOCK_TEXT)
		cur_block = NULL;
	...
	/* Add synthetic space */
    if (add_space && !(dev->flags & FZ_INFO_INHIBIT_SPACES))
...
static void
fz_add_info_char(fz_context *ctx, fz_info_device *dev, fz_font *font, int c, int glyph, fz_matrix trm, float adv, int wmode)
{
	/* ignore when one unicode character maps to multiple glyphs */
	if (c == -1)
		return;

    if (!(dev->flags & FZ_INFO_PRESERVE_LIGATURES))
	{
		switch (c)
		{
		case 0xFB00: /* ff */
			fz_add_info_char_imp(ctx, dev, font, 'f', glyph, trm, adv, wmode);
			fz_add_info_char_imp(ctx, dev, font, 'f', -1, trm, 0, wmode);
			return;
		case 0xFB01: /* fi */
			fz_add_info_char_imp(ctx, dev, font, 'f', glyph, trm, adv, wmode);
			fz_add_info_char_imp(ctx, dev, font, 'i', -1, trm, 0, wmode);
			return;
		case 0xFB02: /* fl */
			fz_add_info_char_imp(ctx, dev, font, 'f', glyph, trm, adv, wmode);
			fz_add_info_char_imp(ctx, dev, font, 'l', -1, trm, 0, wmode);
			return;
		case 0xFB03: /* ffi */
			fz_add_info_char_imp(ctx, dev, font, 'f', glyph, trm, adv, wmode);
			fz_add_info_char_imp(ctx, dev, font, 'f', -1, trm, 0, wmode);
		...
		case 0xFB05: /* long st */
		case 0xFB06: /* st */
			fz_add_info_char_imp(ctx, dev, font, 's', glyph, trm, adv, wmode);
			fz_add_info_char_imp(ctx, dev, font, 't', -1, trm, 0, wmode);
			return;
		}
	}

    if (!(dev->flags & FZ_INFO_PRESERVE_WHITESPACE))
	{
		switch (c)
		{
		case 0x0009: /* tab */
	...
	fz_add_info_char_imp(ctx, dev, font, c, glyph, trm, adv, wmode);
}

static void
fz_info_extract(fz_context *ctx, fz_info_device *dev, fz_text_span *span, fz_matrix ctm)
{
	...
	for (i = 0; i < span->len; i++)
	{
		...
		fz_add_info_char(ctx, dev, font, span->items[i].ucs, span->items[i].gid, trm, adv, span->wmode);
	}
}

static void
fz_info_fill_text(fz_context *ctx, fz_device *dev, const fz_text *text, fz_matrix ctm,
	fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *color_params)
{
	fz_info_device *tdev = (fz_info_device*)dev;
	fz_text_span *span;
	if (text == tdev->lasttext)
		return;
	tdev->new_obj = 1;
	for (span = text->head; span; span = span->next)
		fz_info_extract(ctx, tdev, span, ctm);
	fz_drop_text(ctx, tdev->lasttext);
	tdev->lasttext = fz_keep_text(ctx, text);
}

static void
fz_info_stroke_text(fz_context *ctx, fz_device *dev, const fz_text *text, const fz_stroke_state *stroke, fz_matrix ctm,
	fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *color_params)
{
	fz_info_device *tdev = (fz_info_device*)dev;
	fz_text_span *span;
...
static void
fz_info_clip_text(fz_context *ctx, fz_device *dev, const fz_text *text, fz_matrix ctm, fz_rect scissor)
{
	fz_info_device *tdev = (fz_info_device*)dev;
	fz_text_span *span;
	if (text == tdev->lasttext)
...
static void
fz_info_clip_stroke_text(fz_context *ctx, fz_device *dev, const fz_text *text, const fz_stroke_state *stroke, fz_matrix ctm, fz_rect scissor)
{
	fz_info_device *tdev = (fz_info_device*)dev;
	fz_text_span *span;
	if (text == tdev->lasttext)
...
static void
fz_info_ignore_text(fz_context *ctx, fz_device *dev, const fz_text *text, fz_matrix ctm)
{
	fz_info_device *tdev = (fz_info_device*)dev;
	fz_text_span *span;
	if (text == tdev->lasttext)
...
static void
fz_info_fill_image(fz_context *ctx, fz_device *dev, fz_image *img, fz_matrix ctm, float alpha, const fz_color_params *color_params)
{
	fz_info_device *tdev = (fz_info_device*)dev;
...
static void
fz_info_fill_image_mask(fz_context *ctx, fz_device *dev, fz_image *img, fz_matrix ctm,
		fz_colorspace *cspace, const float *color, float alpha, const fz_color_params *color_params)
{
	fz_info_fill_image(ctx, dev, img, ctm, alpha, color_params);
}
...
static void
fz_info_fill_shade(fz_context *ctx, fz_device *dev, fz_shade *shade, fz_matrix ctm, float alpha, const fz_color_params *color_params)
{
	...
	fz_try(ctx)
		fz_info_fill_image(ctx, dev, image, local_ctm, alpha, color_params);
	fz_always(ctx)
...
static void
fz_info_close_device(fz_context *ctx, fz_device *dev)
{
	fz_info_device *tdev = (fz_info_device*)dev;
	fz_info_page *page = tdev->page;
	fz_info_block *block;
	fz_info_line *line;
	fz_info_char *ch;
	...
	for (block = page->first_block; block; block = block->next)
	{
        if (block->type != FZ_INFO_BLOCK_TEXT)
			continue;
...
static void
fz_info_drop_device(fz_context *ctx, fz_device *dev)
{
	fz_info_device *tdev = (fz_info_device*)dev;
	fz_drop_text(ctx, tdev->lasttext);
}
...
/*
	Parse info device options from a comma separated key-value string.
*/
fz_info_options *
fz_parse_info_options(fz_context *ctx, fz_info_options *opts, const char *string)
{
	const char *val;
...
/*
	...
	options: Options to configure the info device.
*/
fz_device *
fz_new_info_device(fz_context *ctx, fz_info_page *page, const fz_info_options *opts)
{
	fz_info_device *dev = fz_new_derived_device(ctx, fz_info_device);

	dev->super.close_device = fz_info_close_device;
	dev->super.drop_device = fz_info_drop_device;
    ...
    if (opts && (opts->flags & FZ_INFO_PRESERVE_IMAGES))
	{
